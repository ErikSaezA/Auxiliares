...
\item \textbf{Variables de salida}: Se considera que todas las variables de estado son medibles:
\[
...
donde $C = I_3$ (matriz identidad $3 \times 3$). Primero calculamos $(sI - A)$:
\begin{align}
sI - A = \begin{bmatrix}
s & -1 & 0 \\[4pt]
...
Calculando $(sI - A)^{-1}$ usando cofactores y luego multiplicando por $B$:
\begin{align}
...
\begin{itemize}
    \item Las primeras dos salidas (posición y velocidad) no dependen directamente de la entrada voltaje, confirmando el desacoplamiento dinámico.
    ...
    \item El polo en $s = -\frac{R(l_1 - l_0)}{N^2 A \mu_0}$ corresponde al subsistema eléctrico.
\end{itemize}
